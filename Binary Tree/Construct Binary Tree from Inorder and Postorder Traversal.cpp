...
private:
    TreeNode* solve(vector<int>& inorder, int inStart, int inEnd, vector<int>& postorder, int postStart, int postEnd, map<int, int>& mp){
        ...
        root->left = solve(inorder, inStart, inRoot - 1, postorder, postStart, postStart + numsOnLeft - 1, mp);
        root->right = solve(inorder, inRoot + 1, inEnd, postorder, postStart + numsOnLeft, postEnd - 1, mp);
